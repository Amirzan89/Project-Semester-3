...
//             foreach($Stmt as $stmt){
            ...
            if($path1 == '/verify/password' && $method == 'GET'){
                ...
                if ($stmt[0]->fetch()) {
                    ...
                    if ($stmt[1]->fetch()) {
                        ...
                        if ($stmt[2]->fetch()) {
                            $stmt[2]->close();
                            $time = Carbon::now('Asia/Jakarta')->subMinutes(15);
                ...
                if ($stmt[0]->fetch()) {
                    ...
                    $stmt[1] = self::$con->prepare($query);
                    $stmt[1]->bind_param('s', $email, $code);
                    $stmt[1]->execute();
                    $name = '';
                    $stmt[1]->bind_result($name);
                    ...
                    if ($stmt[1]->fetch()) {
                        ...
                        if ($stmt[2]->fetch()) {
                            $stmt[2]->close();
                            echo json_encode(['status'=>'success','message'=>'otp anda benar silahkan ganti password']);
                            exit();
                            ...
                            $stmt[3]->close();
                            throw new Exception(json_encode(['status'=>'error','message'=>'code otp expired']));
                        }
                    }else{
                        $stmt[1]->close();
                        throw new Exception(json_encode(['status'=>'error','message'=>'code otp invalid']));
                    }
                }else{
                    $stmt[0]->close();
                    throw new Exception(json_encode(['status'=>'error','message'=>'email invalid']));
                }
            }
        } catch (Exception $e) {
    ...
    public function changePassEmail($data){
        try{
            ...
            $email = $data['email'];
            $pass = $data["pass"];
            $pass1 = $data["pass_confirm"];
            $code = $data['code'];
            $link = $data['link'];
            $desc = $data['description'];
            if($pass !== $pass1){
                throw new Exception(json_encode(['status'=>'error','message'=>'Password Harus Sama']));
            }else{
                if(is_null($link) || empty($link)){
                    if($desc == 'createUser'){
                        ...
                        if ($stmt[0]->fetch()) {
                            $stmt[0]->close();
                            $query = "SELECT id FROM users WHERE BINARY email = ? LIMIT 1";
                            $stmt[1] = self::$con->prepare($query);
                            $stmt[1]->bind_param('s', $email);
                            $stmt[1]->execute();
                            $name = '';
                            ...
                            if ($stmt[1]->fetch()) {
                                $stmt[1]->close();
                                $query = "SELECT id FROM users WHERE BINARY email = ? AND BINARY code = ? LIMIT 1";
                                $stmt[2] = self::$con->prepare($query);
                                $stmt[2]->bind_param('ss', $email, $code);
                                $stmt[2]->execute();
                                $name = '';
                                $stmt[2]->bind_result($name);
                                ...
                                if ($stmt[2]->fetch()) {
                                    $stmt[2]->close();
                                    $time = Carbon::now('Asia/Jakarta')->subMinutes(15);
                                    $query = "SELECT id FROM users WHERE BINARY email = ? AND updated_at >= ? LIMIT 1";
                                    $stmt[3] = self::$con->prepare($query);
                                    $stmt[3]->bind_param('ss', $email, $time);
                                    $stmt[3]->execute();
                                    $name = '';
                                    ...
                                    if ($stmt[3]->fetch()) {
                                        ...
                                        if ($stmt[4]->fetch()) {
                                            ...
                                            if($result){
                                                $stmt[5]->close();
                                                return json_encode(['status'=>'success','message'=>'ganti password berhasil silahkan login']);
                                            }else{
                                                $stmt[5]->close();
                                                throw new Exception(json_encode(['status'=>'error','message'=>'error update password','code'=>500]));
                                            }
                                        }else{
                                            $stmt[4]->close();
                                            throw new Exception(json_encode(['status'=>'error','message'=>'error update password','code'=>500]));
                                        ...
                                        $stmt[4]->close();
                                        throw new Exception(json_encode(['status'=>'error','message'=>'token expired']));
                                    }
                                }else{
                                    $stmt[2]->close();
                                    throw new Exception(json_encode(['status'=>'error','message'=>'Invalid Email']));
                                }
                            }else{
                                $stmt[1]->close();
                                throw new Exception(json_encode(['status'=>'error','message'=>'Invalid Email']));
                            ...
                            $stmt[0]->close();
                            throw new Exception(json_encode(['status'=>'error','message'=>'token invalid']));
                        }
                    }
                }else{
                        ...
                        if ($stmt[1]->fetch()) {
                            ...
                            if ($stmt[2]->fetch()) {
                                ...
                                if ($stmt[3]->fetch()) {
                                    ...
                                    $result = $stmt[4]->execute();
                                    $stmt[4]->close();
                                    throw new Exception(json_encode(['status'=>'error','message'=>'link expired']));
                                }
                            }else{
                                $stmt[2]->close();
                                throw new Exception(json_encode(['status'=>'error','message'=>'Email invalid']));
                            ...
                            $stmt[1]->close();
                            throw new Exception(json_encode(['status'=>'error','message'=>'Invalid Email1']));
                        }
                    }else{
                        $stmt[0]->close();
                        throw new Exception(json_encode(['status'=>'error','message'=>'link expired']));
            ...
            if ($stmt[0]->fetch()) {
                ...
                if($path1 == '/verify/email' && $method == 'GET'){
                    ...
                    if ($stmt[1]->fetch()) {
                        ...
                        $stmt[1]->close();
                        throw new Exception(json_encode(['status'=>'error','message'=>'invalid token']));
                    }
                }
            }else{
                $stmt[0]->close();
                        ...
                        if ($stmt[2]->fetch()) {
                            ...
                            if ($stmt[3]->fetch()) {
                                ...
                                // if(is_null(DB::table('users')->whereRaw("BINARY email = ?",[$email])->update(['email_verified'=>true]))){
                                    ...
                                    $stmt[4]->close();
                                    throw new Exception(json_encode(['status'=>'error','message'=>'error verify email','code'=>500]));
                                }
                            }else{
                                $stmt[3]->close();
                                $query = "DELETE FROM verify WHERE BINARY email = ? AND description = 'verifyEmail'";
                    ...
                    if ($stmt[1]->fetch()) {
                        ...
                        if ($stmt[2]->fetch()) {
                            ...
                            if ($stmt[3]->fetch()) {
                                ...
                                if($result){
                                    $stmt[4]->close();
                                    return json_encode(['status'=>'success','message'=>'verifikasi email berhasil silahkan login']);
                                }else{
                                    $stmt[4]->close();
                                    throw new Exception(json_encode(['status'=>'error','message'=>'error verify email','code'=>500]));
                                }
    ...
    public function logout($data,$uri = null){
        try{
            ...
            $number = $data['number'];
            if(empty($email) || is_null($email)){
                return json_encode(['status'=>'error','message'=>'email empty','code'=>400]);
            }else if(empty($number) || is_null($number)){
                return json_encode(['status'=>'error','message'=>'token empty','code'=>400]);
            }else{
